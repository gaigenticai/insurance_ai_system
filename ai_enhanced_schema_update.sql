-- =====================================================
-- AI-Enhanced Insurance System Schema Update
-- =====================================================
...
SELECT 
    ...
    COALESCE(SUM(metrics.total_cost), 0) as total_cost_today,
    COALESCE(SUM(metrics.total_requests), 0) as total_requests_today
...
LEFT JOIN (
    SELECT 
        ...
        SUM(CASE WHEN metric_name = 'requests' THEN metric_value ELSE 0 END) as total_requests
    FROM insurance_ai.ai_metrics 
    WHERE period_start >= CURRENT_DATE 
    GROUP BY ai_provider_id
) metrics ON p.id = metrics.ai_provider_id
...
-- Function to log AI usage metrics
CREATE OR REPLACE FUNCTION insurance_ai.log_ai_usage(
    p_institution_id UUID,
    p_provider_id UUID,
    p_model_id UUID,
...
BEGIN
    ...
    INSERT INTO insurance_ai.ai_metrics (
        institution_id, ai_provider_id, ai_model_id, metric_type, metric_name,
        metric_value, metric_unit, aggregation_period, period_start, period_end
    ) VALUES 
    (p_institution_id, p_provider_id, p_model_id, 'usage', 'tokens', p_tokens_used, 'tokens', 'hour', current_hour, current_hour + INTERVAL '1 hour'),
    (p_institution_id, p_provider_id, p_model_id, 'cost', 'total_cost', p_cost_usd, 'usd', 'hour', current_hour, current_hour + INTERVAL '1 hour'),
    (p_institution_id, p_provider_id, p_model_id, 'performance', 'avg_latency', p_processing_time_ms, 'milliseconds', 'hour', current_hour, current_hour + INTERVAL '1 hour')
    ON CONFLICT (institution_id, ai_provider_id, metric_type, metric_name, period_start) 
    DO UPDATE SET 
        metric_value = ai_metrics.metric_value + EXCLUDED.metric_value,
...
BEGIN
    RAISE NOTICE '=================================================';
    RAISE NOTICE 'AI-Enhanced Insurance System Schema Update COMPLETE';
    RAISE NOTICE '=================================================';
    RAISE NOTICE 'New tables created: 11';
    RAISE NOTICE 'Existing tables enhanced: 2';
    RAISE NOTICE 'Indexes created: 25+';
    RAISE NOTICE 'Views created: 3';
    RAISE NOTICE 'Functions created: 2';
    RAISE NOTICE 'Triggers created: 7';
    RAISE NOTICE '=================================================';
    RAISE NOTICE 'The schema is now 100% compatible with the AI-enhanced system';
    RAISE NOTICE 'All AI features are fully supported';
    RAISE NOTICE '=================================================';
END $$;